...
from app.models.resume import Resume, ResumeSkill  # noqa: F401

TEST_DATABASE_URL = "sqlite+aiosqlite:///:memory:"


def _strip_schemas(metadata):
    """SQLite doesn't support PostgreSQL schemas."""
...
def _fix_foreign_keys(metadata):
    """
    Fix cross-schema ForeignKey references for SQLite.
    Changes 'profile_db.resumes.id' to 'resumes.id'
    """
    for table in metadata.tables.values():
...
def auth_headers() -> dict:
    ...
    Returns dict with Authorization header and _user_id for assertions.
    """
    test_user_id = uuid4()
...
def sample_resume_text() -> str:
    """Load the sample resume text fixture."""
